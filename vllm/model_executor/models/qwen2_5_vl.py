...
class Qwen2_5_VLMultiModalProcessor(Qwen2VLMultiModalProcessor):
    ...
    def _get_prompt_updates(
        ...
        # 记录处理器和tokenizer信息
        logger.error(
            f"[Qwen2_5_VLMultiModalProcessor._get_prompt_updates] Processor type: {type(hf_processor).__name__}, "
        ...
        # 记录vocab信息
        logger.error(
            f"[Qwen2_5_VLMultiModalProcessor._get_prompt_updates] Vocab size: {len(vocab)}, "
        ...
        if image_token and image_token not in vocab:
            logger.error(f"[Qwen2_5_VLMultiModalProcessor._get_prompt_updates] image_token '{image_token}' not found in vocab!")
        if video_token and video_token not in vocab:
            logger.error(f"[Qwen2_5_VLMultiModalProcessor._get_prompt_updates] video_token '{video_token}' not found in vocab!")
        ...
        try:
            ...
            logger.error(
                f"[Qwen2_5_VLMultiModalProcessor._get_prompt_updates] Placeholder token IDs - "
                f"image: {placeholder['image']}, video: {placeholder['video']}"
            )
        except KeyError as e:
            missing_token = str(e).strip("'")
            logger.error(
                f"[Qwen2_5_VLMultiModalProcessor._get_prompt_updates] KeyError: Token '{missing_token}' not found in vocabulary. "
                ...
                f"containing 'pad': {[k for k in vocab.keys() if 'pad' in k.lower()][:10]}"
            )
            raise
